...
class FullStyler(Styler):
    ...
    def _load_palettes(self):
        ...
        # ]
        self._random_palette = [
            '#0C1618', '#004643', '#3F7267', '#7D9D8B', '#FAF4D3', '#E6D06A', '#DCBE35', '#D1AC00', '#7389AE'
        ]
    ...
    def _x_regex(self, xvals:dict) -> Tuple[dict, dict]:
        # get x's
        x = 0
        xpos = []
        xticks = []
        ...
        for path in xvals:
            ...
            xticks.append(x)
            xpaths.append(path)
            xlabels.append(self._get_regex(path))
    ...
    def get_x_styling(self, xvals:dict) -> Tuple[dict, dict]:
        ...
        if self._styler_settings['xstyle']['type'] == 'regex':
            return self._x_regex(xvals)
