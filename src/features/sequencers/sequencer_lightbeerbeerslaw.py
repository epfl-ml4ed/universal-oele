...
class LightBeerBeerslawSequencer(Sequencer):
    def __init__(self, settings):
        self._settings = dict(settings)
        self._name = 'lightbeerbeersseq'
        self._notation = 'lbbseq'
    ...
    def _load_settings(self):
        path = './configs/datasets/lightbeer_beerslaw.yaml'
        with open(path) as fp:
            setts = yaml.load(fp, Loader=yaml.FullLoader)
            self._settings['lightbeer_beerslaw'] = setts
    ...
    def _get_label_map(self):
        ...
        if self._settings['lightbeer_beerslaw']['data']['label_map'] == '2highlow':
    ...
    def load_all_sequences(self):
        ...
        """
        path = '{}{}canada_dict.pkl'.format(self._settings['paths']['data'], self._settings['lightbeer_beerslaw']['paths']['root'])
        ...
        for idx in sim_dictionary['index']['index_lid']:
            if 'beerslaw' not in sim_dictionary[idx]:
                continue
            
            with open(sim_dictionary[idx]['beerslaw'].replace('../data', self._settings['paths']['data']), 'rb') as fp:
                sim_beerslaw = pickle.load(fp)
            s, a = self._create_vector_sequence(sim_beerslaw)
            if len(s) == 0:
                continue
            indices.append(idx)
    ...
    def _get_label(self, simulation:dict):
        ...
        """
        if self._settings['lightbeer_beerslaw']['data']['label_map'] == '2highlow':
            score = simulation['main_width'] + simulation['main_concentration'] + simulation['main_wavelength']
